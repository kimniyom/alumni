...
class ForgotController extends Controller {
    ...
    public function actionCheck_user() {
        ...
        $query = "SELECT * FROM company_agent WHERE username = '$username' AND email = '$email' AND question = '$question' AND answer = '$answer' ";
        $rs = Yii::app()->db->createCommand($query)->queryRow();
        if ($rs) {
            echo "<label style='color:red;'>ชื่อใช้งาน : </label>" . $rs['username'] . "<br/> <label>รหัสผ่าน : </label>" . $rs['password'];
        } else {
            $query_collegian = "SELECT * FROM collegian WHERE collegian_username = '$username' AND collegian_email = '$email' AND question = '$question' AND answer = '$answer'";
